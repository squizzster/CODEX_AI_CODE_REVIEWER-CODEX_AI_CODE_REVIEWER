...
def test_create_runner_work_space_creates_requested_contents(tmp_path: Path) -> None:
    ...
    final_report_link = workspace / FINAL_MD_REPORT_LINK_NAME
    assert final_report_link.is_symlink()
    assert final_report_link.readlink() == Path(OUTPUT_DIRECTORY_NAME)
    assert final_report_link.resolve() == (workspace / OUTPUT_DIRECTORY_NAME).resolve()
